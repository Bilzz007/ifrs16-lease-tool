...
def run_ifrs16_model(inputs: Dict):
    try:
        ...
        payments = generate_variable_payments(inputs["payment"], inputs["term_months"], annual_cpi_percent=inputs["cpi"])
        ...
        rou_asset = calculate_right_of_use_asset(liability, inputs["direct_costs"], inputs["incentives"])
        ...
        display_qa(tab3, df)
        display_journals(tab4, df, rou_asset, liability, inputs["direct_costs"], inputs["incentives"], inputs["lease_name"])
